...
const UnifiedMultiPaneChart = forwardRef<UnifiedMultiPaneChartHandle, UnifiedMultiPaneChartProps>(
  function UnifiedMultiPaneChart(
    { candles, indicatorData, activeMainIndicators, activeSubIndicators, onCrosshairMove, onVisibleRangeChange },
    ref
  ) {
    ...
    const labelIndex = hoverIndex !== null ? hoverIndex : Math.max(0, candles.length - 1);
    const hasData = candles.length > 0 && labelIndex >= 0 && labelIndex < candles.length;
    const labelTimeStr = hoverTime && hasData ? candles[labelIndex].timeStr : null;
