...
export function useWasmEngine(tickInterval: number = 100): UseWasmEngineReturn {
  ...
  const placeOrder = useCallback(
    (
      ...
      if (!engineAlive.current || !engineRef.current) {
        console.warn('[useWasmEngine] 引擎未就绪');
        return null;
      }

      if (isProcessingRef.current) {
        console.warn('[useWasmEngine] 引擎正忙');
        return null;
      }
      ...
      try {
        ...
        console.error('[useWasmEngine] 开仓失败:', err);
        if (!result) {
          toast.error(
  ...
  const closePosition = useCallback(
    (
      ...
      try {
        ...
        console.error('[useWasmEngine] 平仓失败:', err);
        if (!result) {
          toast.error(
  ...
  const setLeverage = useCallback(
    (leverage: number): boolean => {
      ...
      try {
        ...
        console.error('[useWasmEngine] 设置杠杆失败:', err);
        return false;
      }
    },
    [toast],
  ...
  const resetAccount = useCallback(
    (initialBalance?: number): void => {
      ...
      try {
        ...
        console.error('[useWasmEngine] 重置账户失败:', err);
      }
    },
    [toast],
  );
  ...
  const cancelOrder = useCallback(
    (orderId: string): CancelOrderResult | null => {
      ...
      try {
        ...
        console.error('[useWasmEngine] 取消挂单失败:', err);
        toast.error(
